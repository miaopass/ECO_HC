...
cv::Mat  get_reg_filter(cv::Size sz, cv::Size2f target_sz, const eco_params& params)
{
	...
	for (size_t i = 0; i < reg_window_dft.rows; i++)
		for (size_t j = 0; j < reg_window_dft.cols; j++)
		{
	...
	//cout <<"minv " <<minv <<endl;
	reg_window_dft.at<float>(0, 0) =reg_window_dft.at<float>(0, 0) - sz.area() * minv + params.reg_window_min;
